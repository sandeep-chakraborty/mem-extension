...
function getChatId() {
  ...
  const chatIndex = urlParts.indexOf('chat');
...
function getChatTitle() {
  // Try to get title from the page title (Claude shows it there)
  const pageTitle = document.title;
  if (pageTitle && pageTitle !== 'Claude' && !pageTitle.includes('New chat')) {
    return pageTitle.replace(' — Claude', '').replace(' - Claude', '').trim();
  }

  // Fallback: use first question as title
  const firstMessage = document.querySelector('[data-is-streaming="false"]');
  if (firstMessage) {
...
function extractQuestions() {
  const questions = [];
  // Messages in Claude are usually wrapped in div elements within the chat stream
  // We'll target the wrappers that contain message content
  const messages = document.querySelectorAll('[data-is-streaming="false"]');

  messages.forEach((message, index) => {
    // Check various indicators for user message
    const isUserMessage = message.querySelector('[data-test-render-count]') ||
      message.querySelector('.font-user-message') ||
      message.classList.contains('font-user-message');
  ...
  // Fallback for strict user selection if above fails to find anything (rare)
  if (questions.length === 0) {
    const alternativeMessages = document.querySelectorAll('.font-user-message');
    ...
    <div class="memory-popup-overlay" id="memory-popup-overlay">
      <div class="memory-popup">
        ...
        <div class="memory-popup-info">
          ...
          <p><strong>Platform:</strong> Claude</p>
